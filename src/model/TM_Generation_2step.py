...
class TM_Generation_2step(BaseModel):
    def __init__(self, dataset, model_conf, device):
        super(TM_Generation_2step, self).__init__(dataset, model_conf)
        ...
        self.separate_encoder = model_conf.get('separate_encoder', False)
    ...
    def make_enc_dec_dict(self):
        ...
        net_vocab_idx_in_bert = self.tokenizer.convert_tokens_to_ids(list(self.dataset.netvocab2netidx.keys())) # list of template token ids in BERT

        all_template_tokens = list(self.dataset.netvocab2netidx.keys())
        ...
        for token in all_template_tokens:
            if token.startswith('[C'):
                ...
                self.always_possible_tokens.append(token)
        self.always_possible_tokens_idx = [self.dataset.netvocab2netidx[token] for token in self.always_possible_tokens]
        self.always_possible_tokens_idx = torch.tensor(self.always_possible_tokens_idx).to(self.device)

        self.dec_vocab_idx2enc_vocab_idx = {i: net_vocab_idx_in_bert[i] for i in self.dataset.netidx2netvocab.keys()} # dict 
    ...
    def build_model(self):
        ...
        if self.separate_encoder:
            self.bert_net = AutoModel.from_pretrained(self.pretrained_path)
            self.bert_op = AutoModel.from_pretrained(self.pretrained_path)
            self.bert_net.resize_token_embeddings(len(self.tokenizer))
            self.bert_op.resize_token_embeddings(len(self.tokenizer))
        ...
        self.embedding = nn.Embedding(self.num_net_vocab + self.num_op_vocab, self.bert_conf.hidden_size)
        ...
        self.classify_lastop_layer = nn.Linear(self.bert_conf.hidden_size, self.num_op_vocab)
    ...
    def forward_NET(self, batch_question_id, batch_question_attention, batch_net_id, batch_net_attention, batch_lastop_id):
        '''
        ...
        if self.separate_encoder:
            batch_imq = self.bert_net(batch_question_id, attention_mask=batch_question_attention)[0]  # (batch, seq_len, hidden_size)
        else:
        ...
        lastop_logits = self.classify_lastop_layer(batch_cls)  # (batch, num_op_vocab)

        if self.use_category:
            # Decoding NET using Transformer Decoder
            decoder_lastop_id = batch_lastop_id + self.num_net_vocab  # (batch)
            # remove BOS token and add lastop token
            batch_net_id = torch.cat([decoder_lastop_id.unsqueeze(1), batch_net_id[:, 1:]], dim=1)  # (batch, seq_len)
    ...
    def forward_OP(self, batch_question_id, batch_question_attention, batch_question_type):
        ...
        if self.separate_encoder:
            if 'roberta' in self.pretrained_path:
                batch_imq_net = self.bert_op(batch_question_id, attention_mask=batch_question_attention)[0]
            else:
                batch_imq_net = self.bert_op(batch_question_id, attention_mask=batch_question_attention, token_type_ids=batch_question_type)[0]
        else:
            if 'roberta' in self.pretrained_path:
    ...
    def train_model_per_batch(self, batch_imq_id, batch_imq_attention, batch_imq_net_id, batch_imq_net_attention, 
        ...
        with torch.cuda.amp.autocast(self.grad_scaler.is_enabled()):
            # Model Forward
            # (batch, seq_len, |V|)
            logit_net, logit_lastop = self.forward_NET(batch_imq_id, batch_imq_attention, batch_net_id, batch_net_attention, batch_lastop_id)
            # (batch, seq_len, |V|)
    ...
    def train_model(self, dataset, evaluator, early_stop, logger, config):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for b, batch_idx in enumerate(tqdm(batch_loader, desc="train_model_per_batch", dynamic_ncols=True)):
                ...
                if self.do_mask_imq:
                    batch_imq_id_new, batch_imq_net_id_new = [], []
                    for idx, s in enumerate(batch_indices):
                        ...
                        batch_imq_net_token_idx = batch_imq_net_id[idx]
                        ...
                        # MASK IMQ+NET
                        mask_indices = np.random.choice(candidate_indices, size=int(len(candidate_indices)*self.imq_mask_ratio), replace=False)
                        batch_imq_net_token_idx[mask_indices] = self.tokenizer.convert_tokens_to_ids(self.tokenizer.mask_token)
                        
                        batch_imq_id_new.append(batch_imq_token_idx)
                        batch_imq_net_id_new.append(batch_imq_net_token_idx)
                        
                    batch_imq_id = np.array(batch_imq_id_new)
                    batch_imq_net_id = np.array(batch_imq_net_id_new)
                ...
                batch_lastop_id = np.zeros(batch_imq_net_id.shape)[:, 0]
                for i, toks in enumerate(batch_op_token):
                    ...
                    for j, tok in enumerate(toks):
                        ...
                        batch_lastop_id[i] = dataset.operator2idx[raw_op[j]]

                batch_op_attention = np.zeros(batch_imq_net_id.shape)
                batch_op_attention[batch_op] = 1

                # Get NET id, attention
                max_net_len = max([len(t.split()) for t in batch_net_text]) + 5
                batch_net_id = np.zeros((len(batch_net_text), max_net_len), dtype=np.int64)
                batch_net_attention = np.zeros_like(batch_net_id)
    ...
    def generate_net_beam(self, batch_question_id, batch_question_attention, isEmbed=False):
        ...
        # Get BERT hidden state
            ...
            if self.separate_encoder:
                batch_imq = self.bert_net(batch_question_id[b].unsqueeze(0), attention_mask=batch_question_attention[b].unsqueeze(0))[0]  # (1, seq_len, hidden_size)
            else:
            ...
            if self.use_category:
                ...
                # remove BOS token and add lastop token
                pred_net = [[lastop_pred[b].item()] for b in range(batch_size)]
            ...
            for i in range(max_seq_len):
                ...
                logits = tmp_logit_net[-1, :, :]  # the last timestep # (beam, |V|)
    ...
    def generate_net_greedy(self, batch_question_id, batch_question_attention):
        ...
        self.eval()
        if self.separate_encoder:
            batch_imq = self.bert_net(batch_question_id, attention_mask=batch_question_attention)[0]  # (batch_size, seq_len, hidden_size)
        else:
        ...
        for b in range(batch_size):
            for i in range(max_seq_len):
                ...
                tmp_logit_net = self.generate_net_layer(decoder_output)
                logits = tmp_logit_net[:, -1, :]   # the last timestep # (1, |V|)
